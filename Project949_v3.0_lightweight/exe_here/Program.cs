...
namespace DesktopCyberPetNative
{
    ...
    sealed class ChatForm : Form
    {
        ...
        void BuildUi()
        {
            ...
            settings.Controls.Add(localPreset, 0, 15);
            ...
            settings.Controls.Add(save, 0, 16);
            settings.SetColumnSpan(save, 2);
            settings.Controls.Add(test, 0, 17);
        ...
        async Task SendMessage()
        {
            ...
            try
            {
                ...
                apiMessages.Add(new ChatMessage("user", InjectMemoryIntoUserContent(userContent, mcpMemory)));
        ...
        object InjectMemoryIntoUserContent(object userContent, string mcpMemory)
        {
            if (string.IsNullOrWhiteSpace(mcpMemory)) return userContent;
            var prefix = "本轮必须参考的本地/MCP记忆:\n" + mcpMemory.Trim() + "\n\n用户本轮输入:\n";
            var text = userContent as string;
            if (text != null) return prefix + text;
            var array = userContent as object[];
            if (array != null && array.Length > 0)
            {
                var first = array[0] as Dictionary<string, object>;
                if (first != null && first.ContainsKey("type") && Convert.ToString(first["type"]) == "text")
                {
                    first["text"] = prefix + Convert.ToString(first.ContainsKey("text") ? first["text"] : "");
                    return array;
                }
            }
            return prefix + Convert.ToString(userContent ?? "");
        }
        string BuildMcpPrompt(string mcpMemory)
        {
            var builder = new StringBuilder();
            builder.AppendLine("MCP长期记忆。下面是本轮同步后的相关记忆，最多5条。用户询问记忆、检测码、编号、ID、偏好或事实时，必须优先依据这些记忆直接回答；不要说你看不到记忆文件。");
        ...
        string BuildMcpMemorySummary(string userText, string assistantText)
        {
            ...
            {
                var messages = new List<ChatMessage>();
                messages.Add(new ChatMessage("system", "你是桌面AI宠物的MCP长期记忆整理器。只输出一句中文纯文本摘要，不要标题，不要Markdown，不要列表。摘要必须保留对未来对话有用的信息，例如用户偏好、正在做的任务、重要事实、刚确认的设置或问题结论。若本轮没有长期价值，也要用一句话概括本轮最有用的信息。最多90个中文字符。"));
    ...
    static class McpMemoryClient
    {
        ...
        const int MaxRetrievedMemories = 5;
        ...
        public static string ReadMemory(Settings settings, string query)
        {
            ...
            return Compact(MergeMemoryBlocks(directText, mcpText, MaxRetrievedMemories), 1100);
        }
        ...
        static void ImportJsonlIntoDoc(Dictionary<string, List<string>> doc)
        {
            ...
            try
            {
                ...
                foreach (var rawLine in lines)
                {
                    ...
                    {
                        var observation = CleanMemoryLine(Convert.ToString(observations[i]));
                        if (observation.Length == 0) continue;
                        AddLine(doc, ClassifyMemory(observation), observation);
                    }
                }
            }
            catch
        ...
        static void IntegrateTextMemory(string petName, string summary)
        {
            ...
            var category = ClassifyMemory(summary);
            var lines = doc[category];
            var merged = false;
            for (int i = 0; i < lines.Count; i++)
            {
                if (IsSimilarMemory(summary, lines[i]))
                {
                    lines[i] = MergeMemorySentence(summary, lines[i]);
                    merged = true;
                    break;
                }
            }
            if (!merged) AddLine(doc, category, summary);
            TrimMemoryDoc(doc);
            SaveTextMemory(doc);
        }
        ...
        static void AddMemoryBlockLines(List<string> lines, string block)
        {
            ...
            {
                var line = CleanMemoryLine(split[i]);
                if (line.Length == 0) continue;
        ...
        {
            string[] pinned = new string[] { "手动记忆", "用户信息", "重要事实" };
            for (int p = 0; p < pinned.Length && output.Count < maxItems; p++)
            {
                var category = pinned[p];
                if (!doc.ContainsKey(category)) continue;
                var lines = doc[category];
                for (int i = lines.Count - 1; i >= 0 && output.Count < maxItems; i--)
                {
                    var line = CleanMemoryLine(lines[i]);
                    if (line.Length == 0) continue;
                    if (ShouldPinMemory(line, query)) AddOutputMemoryLine(output, category, line);
                }
            }
        }
        ...
        static string GraphToText(Dictionary<string, object> graph, string query, int maxItems)
        {
            ...
            var builder = new StringBuilder();
            var take = Math.Min(maxItems, matches.Count);
            for (int i = 0; i < take; i++)
            {
                var match = matches[i];
                if (builder.Length > 0) builder.AppendLine();
                builder.Append("- ").Append(match.EntityName).Append(": ").Append(match.Observation);
            }
            return builder.ToString().Trim();
        }
        ...
        static List<ObservationMatch> CollectMatches(Dictionary<string, object> graph, string query)
        {
            ...
            foreach (var entityObj in entities)
            {
                ...
                for (int i = 0; i < observations.Length; i++)
                {
                    var observation = CleanMemoryLine(Convert.ToString(observations[i]));
                    if (observation.Length == 0) continue;
                    var score = ScoreObservation(name + " " + type + " " + observation, query);
                    var item = new ObservationMatch { EntityName = name, EntityType = type, Observation = observation, Score = score, Index = index++ };
                    if (score > 0) matches.Add(item);
                    fallback.Add(item);
                }
            }
