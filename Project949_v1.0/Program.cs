...
namespace DesktopCyberPetNative
{
    ...
    sealed class Settings
    {
        ...
        public static Settings Load()
        {
            try
            {
                ...
                var serializer = new JavaScriptSerializer();
                var settings = serializer.Deserialize<Settings>(File.ReadAllText(ConfigPath, Encoding.UTF8)) ?? new Settings();
    ...
    sealed class PetForm : Form
    {
        ...
        {
            var sprite = thinking && speakingSprite != null ? speakingSprite : idleSprite;
    ...
    sealed class ChatForm : Form
    {
        ...
        void BuildUi()
        {
            ...
            providerPreset = new ComboBox { DropDownStyle = ComboBoxStyle.DropDownList };
            providerPreset.Items.AddRange(new object[] { "本地 Ollama - llama3.2", "DeepSeek - v4-flash", "DeepSeek - v4-pro", "自定义 OpenAI-compatible" });
            providerPreset.SelectedIndexChanged += delegate { ApplySelectedProviderPreset(false); };
            protocol = new ComboBox { DropDownStyle = ComboBoxStyle.DropDownList };
            protocol.Items.AddRange(new object[] { "http", "https" });
            host = new TextBox();
            petName = new TextBox();
            port = new TextBox();
            path = new TextBox();
            model = new TextBox();
            apiKey = new TextBox { UseSystemPasswordChar = true };
            ...
            settings.Controls.Add(localPreset, 0, 11);
            ...
            settings.Controls.Add(save, 0, 12);
            settings.SetColumnSpan(save, 2);
            settings.Controls.Add(test, 0, 13);
        ...
        {
            if (providerPreset == null || providerPreset.SelectedIndex < 0 || loadingSettings) return;

            SaveCurrentApiPresetFor(state.Settings.Provider);

            var selected = providerPreset.SelectedIndex;
            loadingSettings = true;
            if (selected == 0)
            {
                protocol.SelectedItem = "http";
                host.Text = DetectOllamaHost();
                port.Text = "11434";
                path.Text = "/v1/chat/completions";
                model.Text = "llama3.2:latest";
                apiKey.Text = "";
                state.LastBubble = "已切换到本地 Ollama llama3.2。";
            }
            else if (selected == 1)
            {
                protocol.SelectedItem = "https";
                host.Text = "api.deepseek.com";
                port.Text = "443";
                path.Text = "/chat/completions";
                model.Text = "deepseek-v4-flash";
                state.LastBubble = "已切换到 DeepSeek v4-flash。";
            }
            else if (selected == 2)
            {
                protocol.SelectedItem = "https";
                ...
                state.LastBubble = "已切换到 DeepSeek v4-pro。";
            }

            var providerId = ProviderId(selected);
            var saved = state.Settings.FindApiPreset(providerId);
            if (saved != null)
            {
                protocol.SelectedItem = string.IsNullOrWhiteSpace(saved.Protocol) ? Convert.ToString(protocol.SelectedItem ?? "http") : saved.Protocol;
                host.Text = saved.Host ?? host.Text;
                port.Text = saved.Port ?? port.Text;
                path.Text = saved.Path ?? path.Text;
                model.Text = saved.Model ?? model.Text;
            ...
            if (showMessage)
            {
                var keyHint = selected == 0 ? "本地 Ollama 不需要 API Key。" : "请在 API Key 输入框填入你自己的密钥，然后点保存配置。";
                MessageBox.Show(this, "已套用所选 AI 入口。\n\n" + protocol.SelectedItem + "://" + host.Text + ":" + port.Text + path.Text + "\nmodel: " + model.Text + "\n\n" + keyHint, "AI 入口");
            }
        }
        ...
        void SaveCurrentApiPresetFor(string provider)
        {
            ...
            state.Settings.Protocol = Convert.ToString(protocol.SelectedItem ?? "http");
        ...
        void LoadSettingsToUi()
        {
            ...
            loadingSettings = true;
            providerPreset.SelectedIndex = ProviderIndex(s.Provider);
            protocol.SelectedItem = s.Protocol;
            if (protocol.SelectedIndex < 0) protocol.SelectedIndex = 0;
        ...
        {
            if (provider == "deepseek-v4-flash" || provider == "deepseek-chat") return 1;
            if (provider == "deepseek-v4-pro" || provider == "deepseek-reasoner") return 2;
        ...
        {
            if (index == 1) return "deepseek-v4-flash";
            if (index == 2) return "deepseek-v4-pro";
            if (index == 3) return "custom-openai-compatible";
            return "local-ollama-llama32";
        }
        void SaveSettingsFromUi()
        {
            if (loadingSettings || providerPreset == null) return;
            var s = state.Settings;
            s.Provider = ProviderId(providerPreset.SelectedIndex);
            s.PetName = string.IsNullOrWhiteSpace(petName.Text) ? "Cyber Pet" : petName.Text.Trim();
            s.Protocol = Convert.ToString(protocol.SelectedItem ?? "http");
        ...
        {
            TextBox[] boxes = new TextBox[] { petName, host, port, path, model, apiKey, persona };
        ...
        async Task SendMessage()
        {
            ...
            try
            {
                ...
                apiMessages.Add(new ChatMessage("system", BuildMemoryPrompt(originalChatMemory, originalUserOpinion)));
                    ...
                    ApiClient.SendStreaming(state.Settings, apiMessages, delegate(string delta)
                    {
                        ...
                        foreach (char ch in delta)
                        {
                            ...
                            Invoke(new Action(delegate
                            {
                                ...
                                var cleaned = CleanPlainText(replyBuilder.ToString());
    ...
    static class ApiClient
    {
        ...
        {
            if (settings.Provider == "deepseek-v4-pro" || settings.Model == "deepseek-v4-pro")
            {
        ...
        static string ParseReply(JavaScriptSerializer serializer, string response)
        {
            ...
            if (root.ContainsKey("message")) return Convert.ToString(root["message"]);
            return response;
        }
